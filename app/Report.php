...
//            $elements = $atom->xml->xpath('//component[@type="figure"]/file/@src[starts-with(., "suggested/")]');
    ...
    public static function suggestedImageStats($productId, $filters) {
        ...
        $deactivatedStatusId = Status::getDeactivatedStatusId($productId)->id;

        //for implemented images
        $implementedQuery = Atom::select('id', 'entity_id', 'alpha_title', 'domain_code', 'xml')
                ->where('status_id', '!=', $deactivatedStatusId)
                ->whereIn('id', function ($q) {
                    Atom::buildLatestIDQuery(null, $q);
                })
                ->where('xml', 'like', '%<file src="suggested/%');

        if(is_array($filters) && isset($filters['domain_code'])) {
            $implementedQuery->where('domain_code', '=', $filters['domain_code']);
        }
        ...
        $commentsQuery = Comment::rightJoin('atoms', 'comments.atom_entity_id', '=', 'atoms.entity_id')
                ->where('atoms.status_id', '!=', $deactivatedStatusId)
                ->whereIn('atoms.id', function ($q) {
                    Atom::buildLatestIDQuery(null, $q);
                })
                ->where('comments.text', 'like', '%type="figure"%');

        if(is_array($filters) && isset($filters['domain_code'])) {
            $commentsQuery->where('atoms.domain_code', '=', $filters['domain_code']);
        }
            ...
            if ($imageStatus != 'implemented'){  //for pending, accepted, rejected images
                ...
                if ($figureNodes){
                    foreach ($figureNodes as $figureNode){
                        ...
                        $file = isset($figureNode['file']['@attributes']['src']) ?
                                $figureNode['file']['@attributes']['src'] :
                                '';
                        $credit = isset($figureNode['credit']) ? $figureNode['credit'] : '';
                        $title = isset($figureNode['comp_title']) ? $figureNode['comp_title'] : '';
                        $label = isset($figureNode['label']) ? $figureNode['label'] : '';
                        ...
                        $stats[] = [
                            ...
                            'domain_code' => $comment['domain_code'],
                            'availability' => $availability,
                            'file' => $file,
                            'caption' => $caption,
                            'credit' => $credit,
    ...
    */
    ...
    protected static function _countPerDomain($productId) {
        ...
        $totalNumSql = 'SELECT domain_code, count(id) FROM atoms
            WHERE id IN (
                ...
                WHERE product_id = :productId
                    AND molecule_code IS NOT NULL GROUP BY entity_id
            )
            GROUP BY domain_code
            ORDER BY domain_code';

        $totalNumQuery = DB::select(DB::raw($totalNumSql), $queryValues);
        ...
        $reviewedNumSql = 'SELECT a.domain_code, count(a.id) FROM atoms a
            ...
            WHERE a.id IN (
                SELECT MAX(id) FROM atoms WHERE product_id = :productId AND molecule_code IS NOT NULL GROUP BY entity_id
            )
                AND ass.id IN (
        ...
        $reviewedNumQuery = DB::select(DB::raw($reviewedNumSql), $queryValues);
        ...
        $withCommentNumQuery = DB::select(DB::raw($withCommentNumSql), $queryValues);
    ...
    protected static function _countOfLegacyImage($productId) {
        ...
        $sql = 'SELECT id, entity_id, molecule_code, alpha_title, xml FROM atoms
            ...
            AND deleted_at IS NULL
            AND status_id != :statusId';
        $query = DB::select(DB::raw($sql), $queryValues);
